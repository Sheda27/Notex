...
TextEditingController eddisc = TextEditingController();
Mydb dTb = Mydb();
int? x;
...
class _EditTodoState extends State<EditTodo> {
  ...
  void initState() {
    super.initState();
    task = Get.arguments as ToDo;
    edtitle.text = task.toDoTitle!;
    eddisc.text = task.toDoDisc!;
    // selectedPriority = task.toDoPriority.toString();

    log(
      'recived: ${task.toDoTitle} -------- recived: ${task.toDoDisc}-------- recived: {task.toDoPriority}',
    );
  }
            ...
            Card(
              child: Row(
                ...
                children: [
                  ElevatedButton(
                    onPressed: () async {
                      int result = await dTb.updateDB('todos', {
                        "title": edtitle.text,
                        "discribe": eddisc.text,
                        "date": format,
                        "priority": x,
                      }, "id = ${task.toDoId}");
                      log('todo-----------$result');
